...
function dean_addEvent(e, t, o) {
    if (e.addEventListener) e.addEventListener(t, o, !1);
    else {
        o.$$guid || (o.$$guid = dean_addEvent.guid++), e.events || (e.events = {});
        var n = e.events[t];
        n || (n = e.events[t] = {}, e["on" + t] && (n[0] = e["on" + t])), n[o.$$guid] = o, e["on" + t] = handleEvent
    }
}

function removeEvent(e, t, o) {
    e.removeEventListener ? e.removeEventListener(t, o, !1) : e.events && e.events[t] && delete e.events[t][o.$$guid]
}
...
function handleEvent(e) {
    ...
    for (var n in o) this.$$handleEvent = o[n], !1 === this.$$handleEvent(e) && (t = !1);
    return t
}
...
if (sorttable = {
        ...
        makeSortable: function(e, o) {
			var q = e;
            if (isscr = null != o, 0 == e.getElementsByTagName("thead").length && (the = document.createElement("thead"), the.appendChild(e.rows[0]), e.insertBefore(the, e.firstChild)), null == e.tHead && (e.tHead = e.getElementsByTagName("thead")[0]), 1 == e.tHead.rows.length) {
                sortbottomrows = [];
                for (var t = 0; t < e.rows.length; t++) - 1 != e.rows[t].className.search(/\bsortbottom\b/) && (sortbottomrows[sortbottomrows.length] = e.rows[t]);
                if (sortbottomrows) {
                    for (null == e.tFoot && (tfo = document.createElement("tfoot"), e.appendChild(tfo)), t = 0; t < sortbottomrows.length; t++) tfo.appendChild(sortbottomrows[t]);
                    delete sortbottomrows
                }
                for (headrow = e.tHead.rows[0].cells, t = 0; t < headrow.length; t++) headrow[t].className.match(/\bsorttable_nosort\b/) || (mtch = headrow[t].className.match(/\bsorttable_([a-z0-9]+)\b/), mtch && (override = mtch[1]), mtch && "function" == typeof sorttable["sort_" + override] ? headrow[t].sorttable_sortfunction = sorttable["sort_" + override] : headrow[t].sorttable_sortfunction = sorttable.guessType(e, t, o), headrow[t].sorttable_columnindex = t, headrow[t].sorttable_tbody = null != o ? o.tBodies[0] : e.tBodies[0], dean_addEvent(headrow[t], "click", sorttable.innerSortFunction = function(e) {
                    if (-1 != this.className.search(/\bsorttable_sorted\b/)) return -1 == q.className.search(/\bnoautosort\b/) && sorttable.reverse(this.sorttable_tbody), this.className = this.className.replace("sorttable_sorted", "sorttable_sorted_reverse"), this.removeChild(document.getElementById("sorttable_sortfwdind")), sortrevind = document.createElement("span"), sortrevind.id = "sorttable_sortrevind", sortrevind.innerHTML = stIsIE ? '&nbsp<font face="webdings">5</font>' : "&nbsp;&#x25B4;", void this.appendChild(sortrevind);
                    if (-1 != this.className.search(/\bsorttable_sorted_reverse\b/)) return -1 == q.className.search(/\bnoautosort\b/) && sorttable.reverse(this.sorttable_tbody), this.className = this.className.replace("sorttable_sorted_reverse", "sorttable_sorted"), this.removeChild(document.getElementById("sorttable_sortrevind")), sortfwdind = document.createElement("span"), sortfwdind.id = "sorttable_sortfwdind", sortfwdind.innerHTML = stIsIE ? '&nbsp<font face="webdings">6</font>' : "&nbsp;&#x25BE;", void this.appendChild(sortfwdind);
                    if (theadrow = this.parentNode, forEach(theadrow.childNodes, function(e) {
                            1 == e.nodeType && (e.className = e.className.replace("sorttable_sorted_reverse", ""), e.className = e.className.replace("sorttable_sorted", ""))
                        }), sortfwdind = document.getElementById("sorttable_sortfwdind"), sortfwdind && sortfwdind.parentNode.removeChild(sortfwdind), sortrevind = document.getElementById("sorttable_sortrevind"), sortrevind && sortrevind.parentNode.removeChild(sortrevind), this.className += " sorttable_sorted", sortfwdind = document.createElement("span"), sortfwdind.id = "sorttable_sortfwdind", sortfwdind.innerHTML = stIsIE ? '&nbsp<font face="webdings">6</font>' : "&nbsp;&#x25BE;", this.appendChild(sortfwdind), 0 == isscr) {
							if (-1 == q.className.search(/\bnoautosort\b/)) {
								row_array = [], col = this.sorttable_columnindex, rows = this.sorttable_tbody.rows;
								for (var t = 0; t < rows.length; t++) row_array[row_array.length] = [sorttable.getInnerText(rows[t].cells[col]), rows[t]];
								for (row_array.sort(this.sorttable_sortfunction), tb = this.sorttable_tbody, t = 0; t < row_array.length; t++) tb.appendChild(row_array[t][1]);
								delete row_array
							}
                    }
                }))
        ...
        guessType: function(e, t, o) {
            var n = e.tBodies[0];
            if (o && (n = o.tBodies[0]), null != n && 0 == n.rows.length) return sorttable.sort_alpha;
            if (sortfn = sorttable.sort_alpha, null != n) {
                if (text = sorttable.getInnerText(n.rows[0].cells[t]), "" != text) {
                    if (text.match(/^-?[£$¤]?[\d,.]+%?$/)) return sorttable.sort_numeric;
                    if (possdate = text.match(sorttable.DATE_RE), possdate) {
                        if (first = parseInt(possdate[1]), second = parseInt(possdate[2]), 12 < first) return sorttable.sort_ddmm;
        ...
        getInnerText: function(e) {
            if (!e) return "";
            if (hasInputs = "function" == typeof e.getElementsByTagName && e.getElementsByTagName("input").length, void 0 === e.getAttribute || "function" != typeof e.getAttribute) return "";
            if ("input" == e.nodeName.toLowerCase() && "checkbox" == $("#" + e.id).attr("type")) return 1 == $("#" + e.id).is(":checked") ? "false" : "true";
            if (null != e.getAttribute("sorttable_customkey")) return e.getAttribute("sorttable_customkey");
            ...
            switch (e.nodeType) {
                ...
                case 1:
                case 11:
                    for (var t = "", o = 0; o < e.childNodes.length; o++) t += sorttable.getInnerText(e.childNodes[o]);
        ...
        reverse: function(e) {
			if (e) {
                ...
                for (t = newrows.length - 1; 0 <= t; t--) e.appendChild(newrows[t]);
                delete newrows
            }
        ...
        shaker_sort: function(e, t) {
            for (var o = 0, n = e.length - 1, r = !0; r;) {
                r = !1;
                for (var s = o; s < n; ++s)
                    if (0 < t(e[s], e[s + 1])) {
                        var a = e[s];
                        e[s] = e[s + 1], e[s + 1] = a, r = !0
                    } if (n--, !r) break;
                for (s = n; o < s; --s) t(e[s], e[s - 1]) < 0 && (a = e[s], e[s] = e[s - 1], e[s - 1] = a, r = !0);
                o++
            }
        }
...
window.onload = sorttable.init, dean_addEvent.guid = 1, fixEvent.preventDefault = function() {
    ...
    this.cancelBubble = !0
}, Array.forEach || (Array.forEach = function(e, t, o) {
    for (var n = 0; n < e.length; n++) t.call(o, e[n], n, e)
}), Function.prototype.forEach = function(e, t, o) {
    for (var n in e) void 0 === this.prototype[n] && t.call(o, e[n], n, e)
}, String.forEach = function(o, n, r) {
    Array.forEach(o.split(""), function(e, t) {
        n.call(r, e, t, o)
    })
};
var forEach = function(e, t, o) {
    if (e) {
        var n = Object;
        if (e instanceof Function) n = Function;
        else {
            if (e.forEach instanceof Function) return void e.forEach(t, o);
            "string" == typeof e ? n = String : "number" == typeof e.length && (n = Array)
        }
        n.forEach(e, t, o)
    }
};
