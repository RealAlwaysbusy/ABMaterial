...
//                                url = dl.options.url + '?' + $.param({id: id});
				...
				*/
				if (ttl) {
					if (tou) {
						Tipped.create('#' + event.id, ttl, { position: 'top', maxWidth: 250, showOn: 'touchstart', hideOnClickOutside: true, skin: ttlskin });
					} else {
						Tipped.create('#' + event.id, ttl, { position: 'top', maxWidth: 250, hideOnClickOutside: true, skin: ttlskin });
					}
				}
