...
(function($) {
    ...
    function Plugin($container, options) {
		...
		this.parentid = $container.parent().attr('id') ;
        ...
        function _initialize() {
			...
			if (self.options.color!='') {
				self.colorHex = self.options.color;
        ...
        function _getColorCanvas(event) {
            if(mouseIsDown) {
				...
				b4j_raiseEvent('page_parseevent', {'eventname': self.parentid + '_changed','eventparams':'color','color': self.colorHex});				
            }
        }
