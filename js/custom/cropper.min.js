...
function UploadCropper(canvas,fileName, handler) {
	var ext = "jpeg";
	var Pic = canvas.toDataURL('image/jpeg');
	Pic = Pic.replace('data:image/' + ext + ';base64,', '');
    var boundary = '------------------------------';
    var disposition = 'form-data; name="imageFromCanvas"; filename="' + fileName + '"';
    ...
    xhr.sendAsBinary(['--' + boundary, 'Content-Disposition: ' + disposition, 'Content-Type: ' + 'image/' + ext, '', atob(Pic), '--' + boundary + '--'].join('\r\n'));
}
