            ...
            if (pos) {                          // skip if no position available
                if (event.elements[index])  {   // cached
					...
					if (!event.group) {
						...
						//console.log(this.content.element.closest('.d-dateline'));
						var onClick = 'onclick="buttonclickarray(event, \'\',\'' + opts.eventName + '\',\'' + opts.cursor.toISOString().substring(0,10) + this.content.element.closest('.d-dateline').attr('id') + "-" + event.id + '\')"';
						//console.log(onClick);
						if (event.info) {
							if (event.comm && event.comm!='') {
								ttl = "<b>" + event.info + "</b><br>" + event.comm + "<br><span class='fa fa-gavel' style='padding-right: 5px'></span><a href='#' " + onClick + ">" + ttl;
							} else {	
								ttl = "<b>" + event.info + "</b><br><span class='fa fa-gavel' style='padding-right: 5px'></span><a href='#' " + onClick + ">" + ttl;
							}
						} else {
							if (event.comm && event.comm!='') {
								ttl = "<b>" + event.text + "</b><br>" + event.comm + "<br><span class='fa fa-gavel' style='padding-right: 5px'></span><a href='#' " + onClick + ">" + ttl;
							} else {	
								ttl = "<b>" + event.text + "</b><br><span class='fa fa-gavel' style='padding-right: 5px'></span><a href='#' " + onClick + ">" + ttl;
							}
						}
					} else {
						if (event.info) {
...
//                                url = dl.options.url + '?' + $.param({id: id});
				...
				*/
				if (ttl) {
					if (tou) {
						Tipped.create('#' + event.id, ttl, { position: 'bottom', maxWidth: 250, showOn: 'touchstart', hideOnClickOutside: true });
					} else {
						Tipped.create('#' + event.id, ttl, { position: 'bottom', maxWidth: 250, hideOnClickOutside: true });
					}
				}
    ...
    $.widget("sjaakp.dateline", {
        ...
        _create: function() {
			...
			if (opts.info) {
				var clickBtn = '';
				if (opts.clickBtn) {
					var onClick = 'onclick="buttonclickarray(event, \'\',\'' + opts.eventName + '\',\'' + opts.cursor.toISOString().substring(0,10) + this.element.attr('id') + '\')"';
        ...
        _triggerChange: function()  {
			...
			if (this.options.clickBtn) {
				var onClick = 'onclick="buttonclickarray(event, \'\',\'' + this.options.eventName + '\',\'' + cursor.toISOString().substring(0,10) + this._id.substring(0,this._id.length-4) + '\')"';
