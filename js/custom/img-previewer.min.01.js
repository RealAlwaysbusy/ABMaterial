	...
	function ImgPreviewer(selector, options) {
	    ...
	    function enableMobileScale(wrapper) {
	        ...
	        wrapper === null || wrapper === void 0 ? void 0 : wrapper.addEventListener('touchstart', function (event) {
				...
				_store.clientX = events.clientX;
	            _store.clientY = events.clientY;
	            _store.movable = true;
	            if (events2) {
	        ...
	        document.addEventListener('touchmove', function (event) {
	            ...
	            if (events2) {
	                ...
	                // 最大缩放比例限制
	                if (newScale > mergeOptions.imageZoom.max) {
	                    newScale = mergeOptions.imageZoom.max;
	                }
	                // 记住使用的缩放值
					...
					var diffX = events.pageX - _store.pageX;
	                var diffY = events.pageY - _store.pageY;
					//console.log(store.currentImgElement);
					var moveX = parseInt(store.currentImgElement.offsetLeft) + diffX;
					var moveY = parseInt(store.currentImgElement.offsetTop) + diffY;
					//console.log(moveX);
					//console.log(moveY);
					_store.pageX = events.pageX;
					_store.pageY = events.pageY;
	    ...
	    function handlePreviewerHide() {
	        ...
	        if (isElementInViewport(store.currentClickEl, mergeOptions.bubblingLevel)) {
	            ...
	            store.currentImgElement.addEventListener('transitionend', function (e) {
	                //console.log(e);
